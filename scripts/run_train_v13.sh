#!/usr/bin/env zsh
# Run train_v9_sft.py for v0.13 inside the nemotron-gb10 container.
# Warmstarts from best run15 checkpoint; trains on 16,181-row balanced dataset.
#
...
#
# Override defaults:
#   WARMSTART_ADAPTER=output/adapter_v12_run15_ckpt/step_400 \
#   LEARNING_RATE=1e-4 \
#   MAX_STEPS=600 \
#   bash scripts/run_train_v13.sh
#
...
#
# v0.13 vs v0.12 changes:
#   - TRAIN_FILE: v0.13_train.jsonl (16,181 balanced; top cats capped at 1500,
#     equation_symbolic 1→501 synthetic examples, thin cats repeated to 300)
#   - WARMSTART: best run15 adapter (LR=2e-4 run; already below run14 plateau)
#   - LEARNING_RATE: 1e-4 (lower than run15 2e-4; model is more adapted)
set -euo pipefail
...
RUN_NAME="${RUN_NAME:-v13_$(date +%Y%m%d_%H%M%S)}"
TRAIN_FILE="${TRAIN_FILE:-data/v0.13_train.jsonl}"
WARMSTART_ADAPTER="${WARMSTART_ADAPTER:-output/adapter_v12_run15}"
...
docker rm -f nemotron-trainer-v13 2>/dev/null || true
