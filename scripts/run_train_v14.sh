#!/usr/bin/env zsh
# Run train_v9_sft.py for v0.14 inside the nemotron-gb10 container.
# Warmstarts from run15-step200 (best available); trains on 12,800-row balanced
# v0.14 dataset — all 16 categories including the 3 synthetic additions:
...
# Checkpoints land in output/adapter_<RUN_NAME>_ckpt/ on the host.
#
# v0.14 vs v0.13 changes:
#   - TRAIN_FILE: v0.14_train.jsonl (12,800 rows, all 16 cats, synthetic fills
#     spelling / eq_num_deduce / eq_num_guess absent from v0.13)
#   - MAX_STEPS: 500 (~0.625 epochs; StratifiedSFT sees each new-cat example once)
#   - WARMSTART: run15-step200 (run16 stopped at step 17 — not worth keeping)
set -euo pipefail
...
RUN_NAME="${RUN_NAME:-v14_$(date +%Y%m%d_%H%M%S)}"
TRAIN_FILE="${TRAIN_FILE:-data/v0.14_train.jsonl}"
WARMSTART_ADAPTER="${WARMSTART_ADAPTER:-output/adapter_v12_run15_step200}"
...
docker rm -f nemotron-trainer-v14 2>/dev/null || true
